  ...
  ###### DUP INV DEL for filtering
	if(tipe=="DUP" | tipe=="INV" | tipe=="DEL" | tipe=="sINS"){
      ...
      #cat(RL)
        ...
        pvals.del <-  Pvaldist(cur.del[["nbRP"]] , cur.del[["cov"]], cur.del[["SV_size_max"]], IS.emp)
...
# 			  cur.del[["cov"]] <- cur.del[["cov"]]/(RL/5)
    ...
    detection3 <- merge(detection, clusterAll.freq, by = c("(pair of) chromosome(s)", "nbRP"))
    ...
    probs = sapply(1:length(detection3[,1]), function(x) {
      ...
      nbRP <- as.numeric(detection3[x,"nbRP"])
      ...
      if(tipe=="INV"){
        nbComb <- choose(n,round(2*nbRP))
      } else {
        nbComb <- choose(n,nbRP)
      }
      nbComb <- choose(n,nbRP)
    ...
    ## detectionN$nAB <- paste(detectionN$nbA, detectionN$nbB, sep="-") #version with individual junctions
	  #if(debug==TRUE) {print(head(detectionN ))}
  	detection3 <- merge(detectionN, clusterAll.freq, by = c("chr", "nbRP"))
